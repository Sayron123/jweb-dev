...
const I18N: I18NTranslations = {
    ...
    ja: {
        ...
        common: { loading: '読み込み中...', success: '成功', error: 'エラー', warning: '警告', info: '情報' }
    },
    ko: {
...
function getNested(obj: any, path: string): string | null {
    return path.split('.').reduce((o: any, k: string) => (o && o[k] !== undefined ? o[k] : null), obj);
}
...
function applyLanguage(lang: LanguageCode): void {
    ...
    document.querySelectorAll('[data-i18n-placeholder]').forEach((el) => {
        ...
        if (key) {
            const val = getNested(t, key);
            if (val != null && el instanceof HTMLInputElement) {
                el.placeholder = val;
            }
        }
    });
...
window.setLanguage = function(lang: string): void {
    const validLang = (I18N[lang as LanguageCode] ? lang : 'en') as LanguageCode;
    window._currentLang = validLang;
    document.documentElement.lang = validLang === 'zh' ? 'zh-Cn' : validLang === 'ja' ? 'ja' : validLang === 'ko' ? 'ko' : 'en';
    applyLanguage(validLang);
    try {
        localStorage.setItem('hexgate-lang', validLang);
    } catch (e) {
...
document.addEventListener('DOMContentLoaded', function(): void {
    const saved = typeof localStorage !== 'undefined' ? localStorage.getItem('hexgate-lang') : null;
    const lang = (saved && I18N[saved as LanguageCode] ? saved : 'en') as LanguageCode;
